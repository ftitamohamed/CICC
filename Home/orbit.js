...
for (let i = 0; i < numModels; i++) {
  gltfLoader.load('./models/scene.gltf', (gltf) => {
    ...
    // Scale model 0 bigger than the rest
    if (i === 2) {
      model.scale.set(modelSize * 1.5, modelSize * 1.5, modelSize * 1.5); // Increase size for model 0
    } else {
      model.scale.set(modelSize, modelSize, modelSize); // Normal size for other models
    }
    model.castShadow = true;
...
for (let i = 0; i < numModels; i++) {
  gltfLoader.load('./models/scene.gltf', (gltf) => {
    ...
    // Save model in the models array for future reference
    models.push(model);

    // Log the position of each model
    console.log(`Model ${i} local position:`, model.position);
  });
}
...
function rotateToModel(index) {
  ...
  // Log the world position of each model after the rotation
  models.forEach((model, i) => {
    const worldPosition = new THREE.Vector3();
    model.getWorldPosition(worldPosition); // Get the world position
    console.log(`Model ${i} world position after rotation:`, worldPosition);
  });
}
...
function draw() {
  ...
  models.forEach(model => {
    ...
    if (worldPosition.distanceTo(targetPosition) < tolerance) {
      model.scale.set(targetSize, targetSize, targetSize); // Scale the model at the target position
    } else {
...
for (let i = 0; i < numModels; i++) {
  gltfLoader.load('./models/T_shirt_gltf.zip.gltf', (gltf) => {
    const model = gltf.scene.children[0];
    const angle = i * angleStep;
